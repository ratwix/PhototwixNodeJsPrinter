...
;(function ($, window, document, undefined) {
	...
	Plugin.prototype = {
		...
		hold: function() {
			panes.eq(current_pane).animate({"transform": "translate(" + (pane_width) + "px," + (pane_width*-1.5) + "px) rotate(20deg)"}, $that.settings.animationSpeed, function () {
				if($that.settings.onHold) {
					$that.settings.onHold(panes.eq(current_pane));
				}
				$that.next();
			});
		},
